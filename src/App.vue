...
const playGame = () => {
    ...
    const gameState =
        playerMove.value - 1 == computerMove.value % 3
            ? 1
            : playerMove.value == computerMove.value
            ? 0
            : -1;
...
onMounted(() => {
    stats.value = JSON.parse(
        localStorage.getItem(statsKey) ?? JSON.stringify(DEFAULT_STATS_BOARD)
    );
});
</script>
...
<template>
    ...
    <div>
        ...
        <div>
            <button
                @click="
                    playerMove =
                        playerMove == Moves.Rock ? Moves.None : Moves.Rock
                "
                :class="{
                    selected: playerMove == Moves.Rock,
                }"
            >
                <img src="../public/img/rock.png" width="30px" />
            </button>
            <button
                @click="
                    playerMove =
                        playerMove == Moves.Paper ? Moves.None : Moves.Paper
                "
                :class="{
                    selected: playerMove == Moves.Paper,
                }"
            >
                <img src="../public/img/paper.png" width="30px" />
            </button>
            <button
                @click="
                    playerMove =
            ...
            >
                <img src="../public/img/scissors.png" width="30px" />
            </button>
        </div>
        <Transition>
            <button
                class="lets_go"
                v-if="playerMove! + Moves.None"
                @click="playGame"
            >
                LET'S GO!
            </button>
    ...
    <Teleport to="body">
        <Results
            v-if="computerMove != Moves.None"
            @close-popup="closeResult"
            :data="{
                player: playerMove,
